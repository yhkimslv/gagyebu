...
(function () {
  const isElectron = typeof window.gagyebu !== 'undefined';
  let timer = null;
  let pending = null;   // 아직 저장되지 않은 데이터

  window.Store = {
    async load() {
      if (isElectron) return await window.gagyebu.loadData();
      try {
        const raw = localStorage.getItem('gagyebu-data');
    ...
    saveNow(data) {
      ...
      timer = null;
      pending = null;
      if (isElectron) return window.gagyebu.saveData(data);
      try {
        localStorage.setItem('gagyebu-data', JSON.stringify(data));
      } catch (e) {
