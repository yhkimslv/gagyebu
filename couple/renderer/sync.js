/* 커플 공유 동기화 (Supabase REST)
 * - 로컬 우선(오프라인에서도 동작), 연결되면 올리고(push) 받아오기(pull)
 * - 충돌은 updated_at 이 나중인 쪽이 이김
  ...
  async function syncMeta(data) {
    ...
    if (remote) {
      for (const f of Object.keys(META_COLS)) {
        const col = META_COLS[f];
        const rv = col ? remote[col] : (remote.extra || {})[f];
        if (rv == null) continue;
    ...
    /* 2) 결제수단은 합집합으로 병합한다.
       두 사람이 각자 카드를 등록해도 서로 지우지 않아야 하기 때문이다.
    ...
    /* 어떤 알림을 받을지는 사람마다 다르므로 이름별로 따로 담는다 */
    const prefsOut = { ...(remoteExtra.pushPrefs || {}) };
    if (s.memberName && s.pushPrefs) prefsOut[s.memberName] = s.pushPrefs;
    ...
    /* 3) 이름 목록만은 합집합으로 병합한다.
       각 기기는 '자기 이름'만 넣으므로, 늦게 저장한 쪽이 상대 이름을 지워버리면 안 된다. */
    const retired = s.retiredNames || [];
    const remoteMembers = (remote && Array.isArray(remote.members)) ? remote.members : [];
    const merged = [];
    remoteMembers.concat([s.memberName]).forEach((n) => {
      if (n && retired.indexOf(n) < 0 && merged.indexOf(n) < 0) merged.push(n);
    });
    if (JSON.stringify(merged) !== JSON.stringify(s.members || [])) {
      s.members = merged;
      changed = true;
    }
    const membersChanged = JSON.stringify(merged) !== JSON.stringify(remoteMembers);
    ...
    if (!remote || s.metaDirty || membersChanged || subsChanged) {
      /* 올릴 때도 항목별 시각을 함께 보낸다.
         위 1)에서 상대가 최신인 항목은 이미 그 값을 받아왔으므로, 여기서 되돌아가지 않는다. */
      const tsOut = {};
